...
# -*- coding: utf-8 -*-
"""
File: cost and coverage

@author: zhuoyuanyang & joannachang
"""
...
if response.status_code == 200:
    ...
    print('The data size is:  ',total_rows)  #output{"found_rows":38232,"total_rows":9755427}

# test
# offset_url = url_pitts + "&size="+str(5000)+"&offset="+ str(0)
# offset_response = requests.get(offset_url,headers = headers)
# if offset_response.status_code == 200:
#     data = json.loads(offset_response.content.decode('utf-8'))

# load the data page by page; only have 5，000 rows per time 
...
print(len(df['HCPCS_Cd'].unique())) # see how many HCPCS Code in the table: 1882

# Compute the average cost for each HCPCS code
...
# Mode 1 prep. see what are the most common services and plot the average cost data
...
for idx,row in df_sorted.iterrows():   
    ...
    # check the results
#print(category_tables['X-ray']['HCPCS_Desc'])
categories = list(category_tables.keys())

# Mode 3 prep. match user input with description keywords and show all related charge
...
# Mode 2 prep. Read Highmark coverage file into pandas Dataframe
